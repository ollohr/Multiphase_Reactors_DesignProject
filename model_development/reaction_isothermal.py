...
# F_tot0 = F_CO0 + F_H20 + F_HC0 + F_H2O0
...
#     r = cat_F * a * pco * ph2 / (1 + b * pco)**2
#     rv = r * rho_cat_bed

#     return rv / F_CO0


# conv = np.linspace(0,1,100)

# # target = np.ones_like(conv)*0.8

# vol = solve_ivp(dX_dV,conv,0)
...
def dX_dV(V, X):
    X = X[0]   # because solve_ivp passes y as an array
...
V_span = [0, 100]                 # guess a large enough reactor volume [m^3]
...
#     Ea, R, DbH, dHrxn, U, Tc, a0, b0, p, rho_cat_bed, F0, T = params
...
# vars = F0 
# params  = [Ea, R, DbH, dHrxn, U, Tc, a0, b0, p, rho_cat_bed, F0, T0]
