...
def adiabatic_reactor(V, var, params):
    """
    Computes the mass balance for an adiabatic FTB modeled as a PFR. 

    Args:
        V               (float): Reactor volume [m³].
        var             (np.ndarray): Molar flow rates of species [F_CO, F_H2, F_HC, F_H2O] [mol/s].
        params          (np.ndarray): Model parameters including [a0, b0, Ea, DbH, R, p_bar, cat_F, rho_cat_bed, n_eff, Cp, stoi_mat]
    ...
    a0, b0, Ea, DbH, R, p_bar, cat_F, rho_cat_bed, n_eff, Cp, stoi_mat = params
    ...
    dTdV = ((-dHrxn) * rv) / Cp_flow   # K/m^3

    return np.append(dFdV, dTdV)

V_span = [0, V_fin]
V_eval = np.linspace(0, V_fin, 400)
params = [a0, b0, Ea, DbH, R, p_bar, cat_F, rho_cat_bed, n_eff, Cp, stoi_mat]
vars = np.append(F0, T0)

sol = solve_ivp(adiabatic_reactor,V_span,vars,t_eval=V_eval, method="RK45", args = ([params]))
